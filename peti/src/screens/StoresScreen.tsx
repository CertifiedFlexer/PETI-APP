...
export default function StoresScreen({ navigation }: any) {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch("https://peti-back.onrender.com/api/providers/service/Tienda");
            
            if (!response.ok) {
                if (response.status === 404) {
                    showInfo("No hay tiendas disponibles");
            ...
            if (!json || json.length === 0) {
                showInfo("No hay tiendas disponibles en este momento");
                setData([]);
            } else {
            ...
            if (error.message === 'Network request failed' || error.message.includes('fetch')) {
                ...
                showError("Error inesperado al cargar tiendas");
            }
            setData([]);
        } finally {
            setLoading(false);
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.emptyStateContainer}>
                    ...
                    <Text style={styles.emptyStateTitle}>No hay tiendas disponibles</Text>
...
const styles = StyleSheet.create({
    ...
    cardCategory: { fontSize: 14, color: "#2196F3", fontWeight: "600", marginBottom: 8 },
    ...
    modalRating: { flexDirection: "row", alignItems: "center", marginBottom: 24 },
    modalRatingText: { fontSize: 18, fontWeight: "700", color: "#1A1A1A", marginLeft: 6 },
    modalRatingSubtext: { fontSize: 14, color: "#666", marginLeft: 8 },
